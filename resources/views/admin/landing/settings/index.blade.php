<x-app-layout>
    ...
    <div x-data="{ open: false, setting: { id: null, key: '', value: '', type: 'text', status: 1 } }" class="py-12">
                                ...
                                <tr class="border-t">
                                    ...
                                    <td class="px-3 py-2">
                                        @if($item->type === 'image')
                                            ...
                                            <img src="{{ asset('storage/' . $item->value) }}" class="h-16 rounded shadow" alt="Setting Image">
                                    ...
                                    <td class="px-3 py-2 text-center">
                                        <button 
                                            ...
                                            @click="open = true; setting = {{ $item->toJson() }}">
                                            Edit
                                        </button>
                                    </td>
                                </tr>
        ...
        <div 
            ...
            <div class="bg-white dark:bg-gray-800 p-6 rounded-xl w-full max-w-md shadow-2xl" @click.away="open = false">
                ...
                <h2 class="text-xl font-semibold mb-4" x-text="setting.id ? 'Edit Setting' : 'Tambah Setting'"></h2>

                {{-- Form Handling: Menggunakan x-bind:action untuk URL dinamis --}}
                <form method="POST" 
                      :action="setting.id ? '{{ url('admin/landing/settings') }}/' + setting.id : '{{ route('admin.landing.settings.store') }}'"
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="key" class="w-full border rounded px-3 py-2" x-model="setting.key" required>
                    </div>

                    {{-- Input Type (Hidden/Readonly saat Edit agar tidak merusak logika) --}}
                    <div class="mb-4">
                        <label class="block font-medium mb-1">Type</label>
                        <select name="type" x-model="setting.type" class="w-full border rounded px-3 py-2">
                    ...
                    <template x-if="setting.type === 'image'">
                        <div class="mb-4">
                            ...
                            {{-- Preview Gambar Lama --}}
                            <div class="mt-3" x-show="setting.value && setting.id">
                    ...
                    <template x-if="setting.type !== 'image'">
                        <div class="mb-4">
                            ...
                            <textarea name="value" rows="4" class="w-full border rounded px-3 py-2" x-model="setting.value"></textarea>
                        </div>
                    </template>
                    ...
                    <div class="mb-4">
                         ...
                         <select name="status" x-model="setting.status" class="w-full border rounded px-3 py-2">
