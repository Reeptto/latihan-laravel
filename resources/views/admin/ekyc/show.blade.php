            ...
            <div class="space-y-3">
                @foreach ([
                    ...
                    <div class="flex item-start">
                        <label class="w-36 text-sm font-medium text-gray-700 mt-1">{{ $label }}</label>

                        @if (Str::contains(strtolower($label), 'alamat'))
                            <textarea rows="2" readonly class="flex-1 border border-gray-300 rounded-md p-2 text-sm bg-gray-50">
                                {{ $value }}
                            </textarea>
                        @else
            ...
            <div class="flex flex-wrap gap-6">
                @foreach ([
                    ...
                    <div class="space-y-2">
                        ...
                        @if ($data->$field)
                            <div class="flex-col items-center w-[180px]">
                                <p class="text-xs font-medium mt-1">{{ $label }}</p>
                                <a href="{{ asset('storage/' . $data->$field) }}" target="_blank">
                                    <img src="{{ asset('storage/' . $data->$field) }}"
                                        class="w-[180px] h-[170px] object-cover border rounded-md shadow-sm hover:shadow-lg transition">
                                </a>
                            </div>
                        @endif
                    </div>
